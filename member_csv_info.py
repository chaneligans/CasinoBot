...
async def get_user_csv_currency_amt(member_id):
    user = await get_user_csv_info(member_id)
...
async def get_user_csv_daily_time(member_id):
    user = await get_user_csv_info(member_id)
...
async def set_currency(member_id, amount):
    old_currency_amount = await get_user_csv_currency_amt(member_id)
...
async def set_last_daily_time(member_id):
    new_time = time.time()
    file_name = 'members.csv'
    tempfile = NamedTemporaryFile(mode='w', delete=False)
    fields = ['member_id', 'currency_amt', 'is_admin', 'last_daily_time']
